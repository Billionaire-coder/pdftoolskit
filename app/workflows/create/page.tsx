...
import { Workflow, ArrowRight, Play, File as FileIcon, Download, Loader2, AlertCircle } from 'lucide-react';
...
import { executeWorkflow, WorkflowActionType, WorkflowStep } from '@/lib/workflow-engine';
...
export default function CreateWorkflowPage() {
    ...
    const runWorkflow = async () => {
        ...
        setError(null);
        setResultFiles(null);
        setProgress({ current: 0, total: steps.length, status: 'Starting...' });

        try {
            const results = await executeWorkflow(files, steps, (index, total, status) => {
                setProgress({ current: index + 1, total, status });
            });
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                {resultFiles ? (
                                    ...
                                    <button
                                        ...
                                        className={`px-8 py-3 rounded-xl font-semibold transition-all flex items-center gap-2 ${isProcessing || files.length === 0 || steps.length === 0
                                    ...
                                    >
                                        {isProcessing ? (
                                            <>
                                                ...
                                                Processing Step {progress?.current}/{progress?.total}
                                            </>
                                        ) : (
                                            <>
